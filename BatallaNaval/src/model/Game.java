...
public class Game implements IHit{
    ...
    private boolean validatePosition(int x, int y, boolean isVertical, int type){
        int[][] matrixBoard = boardProgram.getMatrix();

        if(type == 0){
            if(isVertical){
                if((x + AircraftCarrier.SIZE_AIRCRAFT) > 9 ) return false;

                for(int i = 0; i < AircraftCarrier.SIZE_AIRCRAFT; i++)
                    if(matrixBoard[x + i][y] == 1) return false;

            }else{
                if((y + AircraftCarrier.SIZE_AIRCRAFT) > 9 ) return false;

                for(int i = 0; i < AircraftCarrier.SIZE_AIRCRAFT; i++)
                    if(matrixBoard[x][y + i] == 1) return false;
            }
        }else if(type == 1 || type == 2){
            if(isVertical){
                if((x + Vessel.SIZE_VESSEL) > 9) return false;

                for(int i = 0; i < Vessel.SIZE_VESSEL; i++)
                    if(matrixBoard[x + i][y] == 1) return false;
            }else{
                if((y + Vessel.SIZE_VESSEL) > 9) return false;
            ...
            if(isVertical){
                if((x + Submarine.SIZE_SUBMARINE) > 9) return false;

                for(int i = 0; i < Submarine.SIZE_SUBMARINE; i++)
                    if(matrixBoard[x + i][y] == 1) return false;
            }else{
                if((y + Submarine.SIZE_SUBMARINE) > 9) return false;
            ...
            if(isVertical){
                for(int i = 0; i < Boat.SIZE_BOAT; i++)
                    if(matrixBoard[x + i][y] == 1) return false;
            }else{
                for(int i = 0; i < Boat.SIZE_BOAT; i++)
                    if(matrixBoard[x][y + i] == 1) return false;
    ...
    private void initializeProgram(){
        ...
        for(int i = 0; i < 10; i++){
            ...
            boolean rightPosition = false, isVertical = false;
            int posx = -1, posy = -1, vertical = -1;

            while(!rightPosition){
                posx = random.nextInt(Board.SIZE_BOARD);
                posy = random.nextInt(Board.SIZE_BOARD);
                vertical = random.nextInt(2 + 1)+1;
                isVertical = vertical == 1 ? true : false;
                rightPosition = validatePosition(posx, posy, isVertical, i);
            }
